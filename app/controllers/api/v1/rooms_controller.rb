  ...
  module V1
    class RoomsController < ApiController
      ...
      def index
        ...
        user_rooms = current_user.rooms.order(online: :desc).search(params[:search])

        shared_rooms = current_user.shared_rooms.order(online: :desc).search(params[:search]).map do |room|
          room.shared = true
          room
        end
