...
RSpec.describe Api::V1::Admin::ServerRoomsController, type: :controller do
  ...
  describe '#index' do
    ...
    it 'returns the server room status as active if the meeting has started' do
      active_server_room = create(:room, name: 'Meeting')
      ...
      # JSON.parse(response.body)['data'].map do |room|
      #   active_server_rooms << room[:id] if room[:status] == 'Active'
      # end
      expect(JSON.parse(response.body)['data'][0]['status']).to eql('Active')

    end

    # it 'returns the server status as not running if the meeting has not started' do
    #
    # end

    # it 'returns the number of participants in a server room' do
    # end
  end
